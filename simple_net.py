...
VERBOSE = True
N_HID1 = 30
NON_LIN1 = lasagne.nonlinearities.sigmoid
N_HID2 = 15
NON_LIN2 = lasagne.nonlinearities.sigmoid
COMPARE_FLAG = '8'
DELAY = 3
LEARNING_RATE = 2e-5
# REG = 0.002
...
l_in_dropout = lasagne.layers.DropoutLayer(l_in)
l_hid_one = lasagne.layers.DenseLayer(l_in_dropout,
										 num_units=N_HID1,
										 nonlinearity=NON_LIN1)
# l_hid_one = lasagne.layers.DenseLayer(l_in,
# 										 num_units=N_HID1,
# 										 nonlinearity=NON_LIN1)
l_hid_dropout = lasagne.layers.DropoutLayer(l_hid_one)
l_hid_two = lasagne.layers.DenseLayer(l_hid_dropout,
									  num_units=N_HID2,
									  nonlinearity=NON_LIN2)
# l_dropout = lasagne.layers.DropoutLayer(l_out_window)
# l_out = lasagne.layers.DenseLayer(l_dropout,
# 								  num_units=N_CLASSES,
#  								  nonlinearity=lasagne.nonlinearities.softmax)
l_out = lasagne.layers.DenseLayer(l_hid_two,
								  num_units=N_CLASSES,
 								  nonlinearity=lasagne.nonlinearities.softmax)
...
l_output_train = lasagne.layers.get_output(l_out, input)
l_output_test = lasagne.layers.get_output(l_out, input, deterministic=True)
